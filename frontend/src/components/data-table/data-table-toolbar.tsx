...
import { useHotKey } from '@/features/default-table/hooks/use-hot-key'
			...
			<div className='flex flex-wrap items-center gap-2'>
				<TooltipProvider>
					<Tooltip>
						<TooltipTrigger asChild>
							<Button size='sm' variant='ghost' onClick={() => setOpen(prev => !prev)} className='hidden gap-2 sm:flex'>
								{open ? (
									<>
				...
				<div>
					<p className='hidden text-sm text-muted-foreground sm:block'>
						...
						<span className='font-mono font-medium'>{rows.total}</span> row(s){' '}
